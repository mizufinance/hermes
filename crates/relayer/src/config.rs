    ...
    pub fn validate_config(&self) -> Result<(), Diagnostic<Error>> {
        ...
        for chain_config in self.chains.iter() {
            ...
            match chain_config {
                ...
                ChainConfig::Bankd { .. } => { /* no-op for now */ }
            }
        }
...
impl ChainConfig {
    ...
    pub fn packet_filter(&self) -> &PacketFilter {
        match self {
            ...
            Self::Bankd(config) => &config.packet_filter,
        }
    }
    ...
    pub fn max_block_time(&self) -> Duration {
        match self {
            ...
            Self::Bankd(config) => config.max_block_time,
        }
    }
    ...
    pub fn key_name(&self) -> &String {
        match self {
            ...
            Self::Bankd(config) => &config.key_name,
        }
    }
    ...
    pub fn trust_threshold(&self) -> TrustThreshold {
        match self {
            ...
            Self::Bankd(config) => config.trust_threshold,
        }
    }
    ...
    pub fn clear_interval(&self) -> Option<u64> {
        match self {
            ...
            Self::Bankd(config) => config.clear_interval,
        }
    }
    ...
    pub fn query_packets_chunk_size(&self) -> usize {
        match self {
            ...
            Self::Bankd(config) => config.query_packets_chunk_size,
        }
    }
    ...
    pub fn set_query_packets_chunk_size(&mut self, query_packets_chunk_size: usize) {
        match self {
            ...
            Self::Bankd(config) => config.query_packets_chunk_size = query_packets_chunk_size,
        }
    }
    ...
    pub fn excluded_sequences(&self, channel_id: &ChannelId) -> Cow<'_, [Sequence]> {
        match self {
            ...
            Self::Bankd(_config) => Cow::Owned(Vec::new()),
        }
    }
    ...
    pub fn clock_drift(&self) -> Duration {
        match self {
            ...
            Self::Bankd(config) => config.clock_drift,
        }
    }
...
impl<'de> Deserialize<'de> for ChainConfig {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match type_str {
            ...
            "Bankd" => BankdConfig::deserialize(value)
                .map(Self::Bankd)
                .map_err(|e| serde::de::Error::custom(format!("invalid Bankd config: {e}"))),
            //
